...
var CustomDashboardController = {
    ...
    if ( !req.param('indicator') ||
          !req.param('report_type_id'),
					...
					!req.param('end_date') ) {
			return res.json(401, {err: 'indicator, report_type_id, cluster_id, adminRpcode, admin0pcode, organization_tag, admin1pcode, admin2pcode, beneficiaries, start_date, end_date required!'});
		}
	...
	getFilters: function( params ){
      ...
      default: {  },
			...
			cluster_id:  ( params.cluster_id === 'all' || params.cluster_id === 'acbar' )
								? {}
			...
			cluster_id_Native: ( params.cluster_id === 'all' || params.cluster_id === 'acbar' )
								? {}
								: { cluster_id: params.cluster_id },
			cluster_ids_Native: ( params.cluster_ids.includes('all') ||  params.cluster_ids.includes('acbar') )
								? {}
	...
	getIndicator: function ( req, res  ) {
		...
		// parmas, filters
		var params = CustomDashboardController.getParams( req, res );
		var filters = CustomDashboardController.getFilters( params );
    ...
    var filterObject = _.extend({},	filters.default_Native,
      ...
      case 'beneficiaries':
        ...
        // download beneficiairies in CSV format
                ...
                d.project_end_date = moment(d.project_end_date).format('YYYY-MM-DD');
                    ...
                    fs.writeFile(template.dir + template.report + '.csv', csv, function (err) {
                      ...
                      CustomFiles.create(fileDescriptor).exec(function(err, doc) {
                        ...
                        MetricsController.setApiMetrics({
                          dashboard: 'custom_dashboard_generate',
                          theme: params.indicator,
                          url: req.url,
                        }, function (err) { return });
            ...
            var calculate = [];
            if (params.calculate_indicator && Array.isArray(params.calculate_indicator)) {
              params.calculate_indicator.forEach(function (item, i) {
            ...
            if (!calculate.length) {
              calculate.push("$total_beneficiaries");
            }
      ...
      //     }
			...
			case 'markers':
				...
				CustomBeneficiaries.native(function(err, collection) {
                  ...
                  var message = '<h5 style="text-align:center; font-size:1.3rem; font-weight:100;">' + d._id.organization + ' | ' + d._id.project_title + '</h5>'
...
module.exports = CustomDashboardController;
